...
ClipperLib::Path eeToClipper(const Points& ee) {
    ClipperLib::Path clipper;
    clipper.resize(ee.size());
    for (std::size_t i = 0; i < ee.size(); ++i) {
        clipper[i] = ClipperLib::IntPoint(static_cast<ClipperLib::cInt>(ee[i].x * CLIPPER_PRECISION),
                                     static_cast<ClipperLib::cInt>(ee[i].y * CLIPPER_PRECISION));
    }
    return clipper;
}

Points clipperToEE(const ClipperLib::Path& clipper) {
    Points ee(clipper.size());
    for (std::size_t j = 0; j < clipper.size(); ++j) {
        ee[j] = sf::Vector2f(static_cast<float>(clipper[j].X) / CLIPPER_PRECISION,
