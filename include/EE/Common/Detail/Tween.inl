...
template <typename T>
inline T In(const T& a, const T& b, float t) {
    t = 1.0f - std::cos(t * ee::HALFPI);
    return Linear(a, b, t);
}

template <typename T>
inline T Out(const T& a, const T& b, float t) {
    t = std::sin(t * ee::HALFPI);
    return Linear(a, b, t);
}

template <typename T>
inline T InOut(const T& a, const T& b, float t) {
    t = -0.5f * (std::cos(ee::PI * t) - 1.0f);
    return Linear(a, b, t);
}
...
inline T In(const T& a, const T& b, float t) {
    ...
    t -= 1.0f;
    t = (-std::pow(2.0f, 10.0f * t) * std::sin((t - 0.1f) * (2.0f * ee::PI) * 2.5f));
    return Linear(a, b, t);
}
...
inline T Out(const T& a, const T& b, float t) {
    ...
    t =  (std::pow(2.0f, -10.0f * t) * std::sin((t - 0.1f) * (2.0f * ee::PI) * 2.5f) + 1.0f);
    return Linear(a, b, t);
}
...
inline T InOut(const T& a, const T& b, float t) {
    ...
    if (t < 1.0f) {
        t -= 1.0f;
        t = (-0.5f * std::pow(2.0f, 10.0f * t) * std::sin((t - 0.1f) * (2.0f * ee::PI) * 2.5f));
    }
    else {
        t -= 1.0f;
        t = (std::pow(2.0f, -10.0f * t) * std::sin((t - 0.1f) * (2.0f * ee::PI) * 2.5f) * 0.5f + 1.0f);
    }
    return Linear(a, b, t);
}
