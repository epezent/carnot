...
ResourceManager<Font, string> g_fonts;
...
public:
        ...
        m_recvLight(15)
    {
        ...
        m_text.setFont(g_fonts.get("roboto"));
        m_text.setCharacterSize(20);
        m_text.setFillColor(Color::White);
        m_text.setString("Client");
        alignBottomCenter(m_text);
        m_sendLight.setColor(Grays::Gray20);
        m_recvLight.setColor(Grays::Gray20);
        m_sendLight.setPosition(-15, 20);
        m_recvLight.setPosition(15, 20);
    }
    ...
    void draw(RenderTarget& target, RenderStates states) const override {
        ...
        float t;
        t = clamp01(m_sendClock.getElapsedTime().asSeconds() / 0.1f);
        m_sendLight.setColor(Tween::Smoothstep(Greens::Chartreuse, Grays::Gray20, t));
        t = clamp01(m_recvClock.getElapsedTime().asSeconds() / 0.1f);
        m_recvLight.setColor(Tween::Smoothstep(Yellows::Yellow, Grays::Gray20, t));
        target.draw(m_text, states);
        target.draw(m_sendLight, states);
        target.draw(m_recvLight, states);
    }
...
private:
    ...
    mutable SquareShape m_sendLight;
    mutable SquareShape m_recvLight;
    Clock m_sendClock;
    Clock m_recvClock;
